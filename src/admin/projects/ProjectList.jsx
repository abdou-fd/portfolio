...
const ProjectList = () => {
  ...
  const togglePublish = async (id, published) => {
    try {
      ...
      setProjects(
        projects.map((proj) =>
          proj.id === id ? { ...proj, published: !published } : proj
        )
      )
    } catch (err) {
      setError(err.message)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-1">Manage your portfolio projects</p>
        </div>
        <Link
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-lg">
          ...
          <Link to="/admin/projects/new" className="text-accent hover:underline">
                      ...
                      {proj.technologies && (
                        <div className="flex flex-wrap gap-2 mt-2">
                          ...
                          {proj.technologies.length > 3 && (
                            <span className="text-xs px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded">
                              +{proj.technologies.length - 3}
                            </span>
                          )}
                        </div>
